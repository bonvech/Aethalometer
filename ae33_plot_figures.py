...
def average_by_three(datum):
    ...
    return datum.resample("3min").mean().fillna(0) #.rolling(window=3, min_periods=1).mean()
...
def get_data_from_previous_month(name):
    ...
    try:
        ...
        print(f"No file {newname} with data")
        return None
...
def plot_four_figures_from_csv(csvfilename, path_to_figures, nfigs=1):
    print(f"Plot  {nfigs}  figures")
    #print(csvfilename, path_to_figures, nfigs)
    ...
    datum = prepare_data(csvfilename)
    #print(datum.head(2))
    ...
    facecolor = 'white'
    title = csvfilename.split('/')[-1].split('/')[-1].split('.')[-2].split('_')[-1]
    #title = ae_name
